...
if (isset($_POST['add'])) {
    ...
    if(isset($_POST['is_series'])){
      //series test
      $postImdbId = $postImdbId;
      $movieOutputs = run_tmdb_curl($postImdbId);
      echo "<pre>";
      ...
      foreach ($movieOutputs->tv_results as $movieOutput) {
          ...
          foreach ($genres as $category) {
              $catName = json_decode(@file_get_contents("https://api.themoviedb.org/3/genre/{$category}?api_key=bc457c6e89c45bbb2f34a7bdd23688cf"))->name;
          ...
          $video_categories = '25';//$genres_video . ',25';
          $is_kid_friendly = 0;
          ...
          $json = runCurl("https://api.themoviedb.org/3/movie/{$postImdbId}/alternative_titles?api_key=".TMDB_KEY);
          ...
          VALUES ('{$video_name}','{$video_description}','{$video_categories}','{$new_file_name_2}','{$new_file_name_2}','0','{$source}','{$rating}','{$movie->year}','film',\'{$postImdbId}\','{$video_format}','1','{$features}')");
        ...
        $movieOutput = run_tmdb_curl($postImdbId)->movie_results[0];
        if (!empty($movieOutput)) {
            ...
            /*Geners Ends*/
            /*Actors Starts*/
            $json = runCurl("https://api.themoviedb.org/3/movie/{$postImdbId}/credits?api_key=".TMDB_KEY."&external_source=imdb_id");
            ...
            if ($im_url != "") {
                ...
                VALUES ('{$video_name}','{$video_description}','{$video_categories}','{$new_file_name_2}','{$new_file_name_2}','0','{$source}','{$rating}','{$movie->year}','film','{$postImdbId}','{$video_format}','{$features}')");
                $movie_id = $db->insert_id;
                $db->query("INSERT INTO ratings(movie_id,user_id,rating) VALUES ('{$movie_id}','22','{$rating}')");
                foreach ($actors as $actor => $actor_id) {
                    $db->query("INSERT INTO actor_relations(movie_id,actor_id) VALUES ('{$movie_id}','{$actor_id}')");
