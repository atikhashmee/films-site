...
if($delid!=''){
$feedback = $db->query("DELETE FROM my_list WHERE movie_id = '".$delid."'");
if($feedback==1){
  echo "<script>window.location.href = 'http://installer123.ddns.net/my_list.php';</script>";
}
}
?>
      ...
      <div class="my-list">
		...
		if($list_item->type == "episode") {
			$list_item = $db->query("SELECT * FROM episodes WHERE id='".$list_item->movie_id."' LIMIT 1");
			  ...
			  $muviko->newHomeMovieItem_mylist($list_item->id,$list_item->episode_thumbnail,$list_item->episode_name);
		}
		else {
			$list_item = $db->query("SELECT * FROM movies WHERE id='".$list_item->movie_id."' LIMIT 1"); 
      $list_item = $list_item->fetch_object();
          $muviko->newHomeMovieItem_mylist($list_item->id,$list_item->movie_thumb_image,$list_item->movie_name,$list_item->movie_year,$list_item->movie_rating,$list_item->is_series,$list_item->last_season);
		}    
  }
?>
      </div>
