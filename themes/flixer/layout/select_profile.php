...
<div class="content animated fadeIn" onclick="hideSearch();">
  <div class="profile-selection vertical-center">
    <h1><?=$muviko->translate('Who_Is_Watching')?></h1>
    <?php
    while($profile = $profiles->fetch_object()) { ?>
    <div class="profile">
      <a href="?set_profile=true&profile_id=<?=$profile->id?>&profile_name=<?=$profile->profile_name?>&kid=<?=$profile->is_kid?>">
        <div class="avatar">
    ...
    <a href="<?=$muviko->getDomain()?>/manage_profiles.php" class="btn btn-neutral btn-fill btn-lg">
     <?=$muviko->translate('Manage_Profiles')?>
   </a>
 </div>
</div>