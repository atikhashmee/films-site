...
<div class="container animated fadeIn" onclick="hideSearch();">
  <div class="row">
    <div class="col-lg-12">
      <div class="my-list">
        <h1><?php if(empty(ucwords($genre_name,'-'))){
            echo "Series";
            }
            else{
                echo ucwords($genre_name,'-');
            }?></h1>
        <?php
        while($movie_item = $genre_movies->fetch_object()) {
          $muviko->newHomeMovieItem($movie_item->id,$movie_item->movie_thumb_image,$movie_item->movie_name,$movie_item->movie_year,$movie_item->movie_rating,$movie_item->is_series,$movie_item->last_season,$movie_item->watch);
        }
        ?>
      </div>
        ...
        echo '<div class="clearfix"></div><div class="text-center">';
          echo $muviko->paginationCat($total_movies,$paged,$limit);
        echo '</div>';
        ?>
    </div>
  </div>
