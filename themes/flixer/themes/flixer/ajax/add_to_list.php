...
$check = $db->query("SELECT * FROM my_list WHERE movie_id='".$movie_id."' AND user_id='".$user_id."' AND type='episode' LIMIT 1");

if($check->num_rows >= 1) {
	$db->query("DELETE FROM my_list WHERE movie_id='".$movie_id."' AND user_id='".$user_id."' AND type='episode' ");
	echo '<i class="ti-plus"></i> <span>'.$muviko->translate('Add_List').'</span>';
} else { 
	$db->query("INSERT INTO my_list (movie_id,user_id,profile_id,time,type) VALUES ('".$movie_id."','".$user_id."','".$_SESSION['fl_profile']."','".time()."','episode')");
