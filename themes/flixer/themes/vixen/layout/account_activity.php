  ...
  <div class="row">
    <div class="col-lg-12">
      <div class="setting-container">
        <div class="col-lg-12">
          <h1><?=$muviko->translate('Account_Activity')?></h1>
          <p> The latest activity on your account is displayed below. </p>
          <?php if($sessions->num_rows >= 1) { ?>
          <table class="table table-responsive">
            ...
            <tbody>
              <?php while($session = $sessions->fetch_object()) { ?>
              <tr>
                <td> <?=date('F j, Y, g:i A',$session->time)?> </td>
                <td>
                  <?=$muviko->getCountryByIp($session->user_ip)?> <br>
                  <?=$session->user_ip?>
                </td>
                <td> Mac OS X Device </td>
                <td> <?=$muviko->readableSessionStatus($session->is_active)?> </td>
              </tr>
              <? } ?>
            </tbody>
          </table>
          <a href="settings.php" class="btn btn-success btn-fill btn-md"><?=$muviko->translate('Done')?></a>
          <? } ?>
        </div>
      </div>
    </div>
